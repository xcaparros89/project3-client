    ...
    render() {
      return (
        <Consumer>
          ...
          {({ login, signup, user, logout, isLoggedin, guest }) => {
            return (
              <WrappedComponent
                login={login}
                signup={signup}
...
class AuthProvider extends React.Component {
  ...
  login = (user) => {
    ...
    auth
      ...
      .then((data) => {console.log(data.username, 'authProvider'); this.setState({ isLoggedin: true, user:data.username })})
  ...
  render() {
    ...
    return isLoading ? (
      ...
      <Provider value={{ isLoggedin, user, login, logout, signup, guest }}>
        {this.props.children}
      </Provider>
